...
int onnx_shim_init(const char *log_id,
                   OrtEnv **out_env,
                   char **out_error);
void onnx_shim_release_env(OrtEnv *env);

int onnx_shim_create_session_options(OrtSessionOptions **out,
                                     char **out_error);
void onnx_shim_release_session_options(OrtSessionOptions *options);
...
int onnx_shim_enable_provider(OrtSessionOptions *options,
                              const char *provider_name,
                              int64_t flags,
                              char **out_error);

int onnx_shim_create_session_from_path(OrtEnv *env,
                                       const char *model_path,
                                       OrtSessionOptions *options,
                                       OrtSession **out,
                                       char **out_error);
int onnx_shim_create_session_from_buffer(OrtEnv *env,
                                         const void *buf,
                                         size_t len,
                                         OrtSessionOptions *options,
                                         OrtSession **out,
                                         char **out_error);
void onnx_shim_release_session(OrtSession *session);

/* --- Session introspection --- */

int onnx_shim_session_input_count(OrtSession *session,
                                  size_t *out,
                                  char **out_error);
int onnx_shim_session_output_count(OrtSession *session,
                                   size_t *out,
                                   char **out_error);
int onnx_shim_session_input_name(OrtSession *session,
                                 size_t idx,
                                 char **out_name,
                                 char **out_error);
int onnx_shim_session_output_name(OrtSession *session,
                                  size_t idx,
                                  char **out_name,
                                  char **out_error);
...
void onnx_shim_free_name(char *name);
...
int onnx_shim_session_input_type_info(OrtSession *session,
                                      size_t idx,
                                      ...
                                      size_t shape_cap,
                                      char **out_error);
int onnx_shim_session_output_type_info(OrtSession *session,
                                       size_t idx,
                                       int32_t *out_elem_type,
                                       size_t *out_rank,
                                       int64_t *out_shape,
...
int onnx_shim_create_tensor(const void *data,
                            size_t byte_len,
                            const int64_t *shape,
                            size_t rank,
                            int32_t elem_type,
...
int onnx_shim_tensor_data(OrtValue *value,
                          void **out_data,
                          size_t *out_byte_len,
                          char **out_error);
...
int onnx_shim_run(OrtSession *session,
